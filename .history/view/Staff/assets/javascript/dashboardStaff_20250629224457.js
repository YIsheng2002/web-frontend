...
function handleMemberTypeChange(selectElement) {
  const form = selectElement.closest('form'); // Get the current form
  const runnerTypeGroup = form.querySelector('#runnerTypeGroup');
  const runnerTypeDropdown = form.querySelector('#member-runnerType');

  if (selectElement.value === 'runner') {
    runnerTypeGroup.classList.remove('hidden');
  } else {
    runnerTypeGroup.classList.add('hidden');
    if (runnerTypeDropdown) runnerTypeDropdown.value = '';
  }
}
