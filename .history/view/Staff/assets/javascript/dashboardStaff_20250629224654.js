...
function handleMemberTypeChange() {
  const memberType = document.getElementById('memberTypeAdd').value;
  const runnerTypeGroup = document.getElementById('runnerTypeGroupAdd');

  if (memberType === 'runner') {
    runnerTypeGroup.classList.remove('hidden');
  } else {
    runnerTypeGroup.classList.add('hidden');
    document.getElementById('member-runnerTypeAdd').value = ''; // Reset dropdown
  }
}
