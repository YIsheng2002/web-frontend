...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const addTypeSelect = document.querySelector('#addMemberForm #memberType');
  if (addTypeSelect) handleMemberTypeChange(addTypeSelect);

  const editTypeSelect = document.querySelector('#editMemberForm #memberType');
  if (editTypeSelect) handleMemberTypeChange(editTypeSelect);
...
function handleMemberTypeChange(selectElement) {
  const formId = selectElement.closest('form').id;

  let runnerGroup;
  if (formId === 'addMemberForm') {
    runnerGroup = document.getElementById('runnerTypeGroupAdd');
  } else if (formId === 'editMemberForm') {
    runnerGroup = document.getElementById('runnerTypeGroupEdit');
  }

  if (selectElement.value === 'runner') {
