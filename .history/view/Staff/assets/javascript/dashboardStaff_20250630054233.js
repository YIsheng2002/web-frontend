...
async function renderCurrentOrders(orderList) {
  ...
  const unassignedOrders = orderList.filter(order => !order.runner_id);
  const assignedOrders = orderList.filter(order => order.runner_id);

  const renderOrderRow = (order, isUnassigned) => {
    const row = document.createElement('div');
    row.className = isUnassigned ? 'order-row ongoing' : 'order-row.pending';
  ...
  <div>
    <select ${isUnassigned ? '' : 'disabled'}>
      <option disabled ${isUnassigned ? 'selected' : ''}>
        ${isUnassigned ? 'Assign runner' : 'Runner #'+order.runner_id+' - '+order.runner_type}
      </option>
      ${runnerOptions}
    </select>
  </div>
