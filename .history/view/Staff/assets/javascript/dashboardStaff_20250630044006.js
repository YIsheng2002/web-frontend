...
async function renderCurrentOrders(orderList) {
  ...
  orderList.forEach(order => {
    ...
    const runnerOptions = runnerList.map(runner =>
      `<option value="${runner.runner_id}">${runner.runner_id} - ${runner.runner_type}</option>`
    ).join('');
    ...
    row.innerHTML = `
      ...
      <div>${order.delivery_address || 'N/A'}</div>
      <div>
        <select onchange="assignRunner(${order.order_id}, this.value)">
...
async function fetchAvailableRunners() {
  try {
    const response = await fetch('http://localhost/orderManagementSystemFrontend/view/Staff/backend-api/getRunners.php');
    const result = await response.json();
    return result.data || [];
  } catch (error) {
...
async function assignRunner(orderId, runnerId) {
  try {
