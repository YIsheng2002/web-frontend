...
class Utils {
    ...
    static validateForm(formElement) {
        ...
        if (input.name === 'username' && value.length < 3) {
            Utils.showFieldError(input, 'Username must be at least 3 characters');
            isValid = false;
            return;
        }

        // Phone number pattern
        if (input.name === 'member-phonenumber') {
            const phonePattern = /^[0-9]{10,}$/;
            if (!phonePattern.test(value)) {
                Utils.showFieldError(input, 'Phone number must be numeric and at least 10 digits');
                isValid = false;
                return;
            }
        }
        ...
        // Gender not default
        if (input.id === 'memberGender' && value === '') {
            Utils.showFieldError(input, 'Please select a gender');
            isValid = false;
            return;
        }
            
            // Password validation
            if (input.type === 'password' && (input.name === 'password' || input.name === 'member-password')) {
