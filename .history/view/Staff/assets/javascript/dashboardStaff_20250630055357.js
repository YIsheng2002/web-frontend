...
async function renderCurrentOrders(orderList) {
  ...
  const unassignedOrders = orderList.filter(order => !order.runner_id || order.runner_id === 'null' || order.runner_id === null);
  const assignedOrders = orderList.filter(order => order.runner_id && order.runner_id !== 'null' && order.runner_id !== null);

  const renderOrderRow = (order, isUnassigned) => {
    const row = document.createElement('div');
    row.className = isUnassigned ? 'order-row highlighted' : 'order-row ongoing';
