...
async function handleAddMember(event) {
  ...
  ['member-username', 'member-email', 'member-password', 'member-repassword', 'member-phonenumber'].forEach(clearFieldError);
  ...
  const repassword = document.getElementById('member-repassword').value.trim();
  const phone = document.getElementById('member-phonenumber').value.trim();

  let valid = true;

  if (!isValidUsername(username)) {
    highlightFieldError('member-username', 'Username must be between 3-20 characters.');
    valid = false;
  }

  if (!isValidEmail(email)) {
    highlightFieldError('member-email', 'Please enter a valid email address.');
    valid = false;
  }

  if (!isValidPassword(password)) {
    highlightFieldError('member-password', 'Password must be at least 6 characters, include a number and an uppercase letter.');
    valid = false;
  }

  if (password !== repassword) {
    highlightFieldError('member-repassword', 'Passwords do not match.');
    valid = false;
  }

  if (!isValidPhone(phone)) {
    highlightFieldError('member-phonenumber', 'Phone number must be 10-15 digits.');
    valid = false;
  }
...
function validatePasswordMatch() {
  ...
  const confirmPassword = document.getElementById('member-repassword');
...
async function addMemberApi(formData) {
  try {
    ...
    if (!result.success) {
      if (result.usernameExists) highlightFieldError('member-username', 'Username already exists.');
      if (result.emailExists) highlightFieldError('member-email', 'Email already exists.');
      if (result.phoneExists) highlightFieldError('member-phonenumber', 'Phone number already exists.');

      Utils.showNotification(result.message || 'Failed to add member', 'error'); // Add this line for feedback
      return result;
    }
