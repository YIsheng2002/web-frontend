...
async function handleAddMember(event) {
  ...
  if (username.value.trim().length < 3) {
    Utils.showFieldError(username, 'Username must be at least 3 characters long');
    return;
  }
  ...
  const phone = document.getElementById('member-phoneNumber');
  const phonePattern = /^[0-9]{10,}$/;
  if (!phonePattern.test(phone.value.trim())) {
    Utils.showFieldError(phone, 'Please enter a valid phone number (min 10 digits)');
    return;
  }
  // Password match validation
  if (!validatePasswordMatch('member-password', 'member-repassword')) return;
