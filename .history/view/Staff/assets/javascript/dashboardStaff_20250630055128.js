...
async function renderCurrentOrders(orderList) {
  ...
  const unassignedOrders = orderList.filter(order => !order.runner_id || order.runner_id === 'null');
  const assignedOrders = orderList.filter(order => order.runner_id && order.runner_id !== 'null');
