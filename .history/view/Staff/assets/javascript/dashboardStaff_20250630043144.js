...
function showOrderTab(tabId, event) {
  ...
  if (tabId === 'new-orders') {
    ...
    fetchOrders('assigned, preparing, picked up,in transit', tabId);
...
function renderCurrentOrders(orderList) {
  ...
  if (orderList.length === 0) {
    ...
    emptyMessage.innerHTML = `
      <div style="text-align: center; padding: 40px;">
        ...
        <p>Orders will appear here once they are picked up or assigned.</p>
      </div>`;
    container.appendChild(emptyMessage);
    return;
  }
  ...
  orderList.forEach(order => {
    ...
    row.innerHTML = `
      ...
      <div>${order.delivery_address || 'N/A'}</div>
      <div>
        <select class="status-dropdown" onchange="handleStatusChange(${order.order_id}, this.value)">
          ${options}
        </select>
      </div>
    `;
