...
// Basic validation
if (!$role || !$firstName || !$lastName || !$username || !$email || !$password || !$phonenumber || !$gender || !$address) {
...
try {
    ...
    $stmt = $conn->prepare("SELECT COUNT(*) FROM user WHERE phonenumber = ?");
    ...
    // Insert into DB
    $stmt = $conn->prepare("INSERT INTO user (role, firstname, lastname, username, email, password, phonenumber, gender, runner_type, address)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->execute([
        $role,
        $firstName,
